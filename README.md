...
Mission Planner ─────── MAVLink ───────┐
                                       v
Gazebo Sim <── JSON/FDM ──> ArduPilot SITL
    │                                  ^
    │ downward camera                  │ GUIDED velocity setpoint
    v                                  │
YOLOv5 detector ──> target controller ─┘

RadioMaster joystick ──> RC bridge ──> MAVLink
```
...
| TCP 5760 | 필수 | SITL -> MAVProxy | SITL 기본 MAVLink master |
| TCP 5763 | 필수 | YOLO -> SITL | 현재 YOLO GUIDED controller 전용 연결 |
| UDP 14550 | 필수 | MAVProxy -> Mission Planner | GCS telemetry와 명령 |
| UDP 5600 | 필수 | Gazebo -> YOLO | 현재 하향 카메라 H.264 stream |
| UDP 14551 | 선택 | MAVProxy -> RC bridge/보조 client | 조종기 또는 별도 MAVLink client 사용 시 |
| TCP 5762 | 선택 | client -> SITL | 테스트·디버깅용 추가 MAVLink 연결 |

`14550`은 Mission Planner가 Windows에서 실행될 때만 Windows host IP로 전달하면 됩니다. 나머지 내부 포트는 기본적으로 `127.0.0.1`에 유지합니다.
